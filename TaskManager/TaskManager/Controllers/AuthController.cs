...
namespace TaskManager.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public IActionResult Login(LoginDto command)
        {
            ...
            var existingToken = _context.UserTokens.FirstOrDefault(ut => ut.UserId == user.Id);

            if (existingToken != null)
            {
                existingToken.Token = jwtToken;
                existingToken.TokenExp = DateTime.UtcNow.AddMinutes(30); // تنظیم تاریخ انقضای جدید برای توکن قبلی
            }
            else
            {
        ...
        public IActionResult GetMaster()
        {
            ...
            var id = int.Parse(User.Claims.FirstOrDefault(f => f.Type == ClaimTypes.NameIdentifier).Value);
